...
KitRenderColor mem_console_ui_project_color_for_key(const char *project_key) {
    static const KitRenderColor k_palette[] = {
        { 92, 128, 170, 255 },
        { 108, 154, 128, 255 },
        { 168, 138, 92, 255 },
        { 132, 112, 168, 255 },
    ...
    if (!project_key || project_key[0] == '\0') {
        return (KitRenderColor){ 130, 130, 146, 255 };
    }

    while (project_key[i] != '\0') {
        hash ^= (uint32_t)(uint8_t)project_key[i];
        hash *= 16777619u;
        i += 1u;
    }

    index = hash % (uint32_t)(sizeof(k_palette) / sizeof(k_palette[0]));
    return k_palette[index];
}
...
static float wrapped_text_line_step(CoreFontTextSizeTier text_tier) {
    switch (text_tier) {
        case CORE_FONT_TEXT_SIZE_CAPTION:
            return 16.0f;
        case CORE_FONT_TEXT_SIZE_BASIC:
...
CoreResult mem_console_ui_draw_wrapped_text_block(KitUiContext *ui_ctx,
    ...
    line_step = wrapped_text_line_step(text_tier);
    if (line_step < 14.0f) {
        line_step = 14.0f;
    }
    ...
    while (*cursor != '\0' && line_count < max_lines) {
        ...
        line_rect = (KitRenderRect){
            bounds.x + 8.0f,
            bounds.y + 6.0f + ((float)line_count * line_step),
            bounds.width - 16.0f,
            line_step
        };
        result = mem_console_ui_draw_info_line_custom(ui_ctx,
