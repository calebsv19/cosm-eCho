...
static KitRenderColor hud_darkened_surface(KitRenderColor base, float factor, uint8_t alpha) {
    if (factor < 0.0f) {
        factor = 0.0f;
    }
    if (factor > 1.0f) {
...
static void hud_place_cached_rects(MemConsoleState *state,
                                   KitRenderRect bounds,
                                   float edge_margin,
                                   KitRenderVec2 cursor_anchor) {
    float left = bounds.x + edge_margin;
    float top = bounds.y + edge_margin;
    float right = bounds.x + bounds.width - edge_margin;
    float bottom = bounds.y + bounds.height - edge_margin;
    float pad = 10.0f;
    float x;
    float y;
    ...
    x = cursor_anchor.x + pad;
    if (x + state->graph_hud_cache_outer.width > right) {
        x = cursor_anchor.x - state->graph_hud_cache_outer.width - pad;
    }
    if (x < left) {
        x = left;
    }

    y = cursor_anchor.y - state->graph_hud_cache_outer.height - pad;
    if (y < top) {
        y = cursor_anchor.y + pad;
    }
    if (y + state->graph_hud_cache_outer.height > bottom) {
        y = bottom - state->graph_hud_cache_outer.height;
    }
    if (y < top) {
        y = top;
    }

    state->graph_hud_cache_outer.x = x;
    state->graph_hud_cache_outer.y = y;
    state->graph_hud_cache_inner.x = x + 6.0f;
    state->graph_hud_cache_inner.y = y + 6.0f;
}
...
CoreResult mem_console_ui_hud_draw_cached(const KitRenderContext *render_ctx,
    ...
    hud_place_cached_rects(state, bounds, edge_margin, cursor_anchor);
    ...
    hud_bg_color = hud_darkened_surface(hud_bg_color, 0.28f, 174u);
