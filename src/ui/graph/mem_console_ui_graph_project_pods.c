...
static uint32_t graph_hash_u32(uint32_t value) {
    value ^= value >> 16;
    value *= 0x7feb352du;
    value ^= value >> 15;
    value *= 0x846ca68bu;
...
static float graph_hash_unit_float(uint32_t seed) {
    uint32_t hash = graph_hash_u32(seed);
...
static void graph_place_nodes_in_lane(KitGraphStructNodeLayout *layouts,
    ...
    for (idx = 0; idx < count; ++idx) {
        ...
        float stagger_x = (grid_row & 1) ? (cell_w * 0.18f) : -(cell_w * 0.12f);
        float fan_y = (((float)grid_col + 0.5f) - ((float)grid_cols * 0.5f)) * (cell_h * 0.08f);
        float jitter_x;
        float jitter_y;
        uint32_t seed;

        seed = (uint32_t)node_index * 2654435761u;
        seed ^= (uint32_t)idx * 2246822519u;
        jitter_x = (graph_hash_unit_float(seed) - 0.5f) * cell_w * 0.28f;
        jitter_y = (graph_hash_unit_float(seed ^ 0x9e3779b9u) - 0.5f) * cell_h * 0.30f;
        cx += stagger_x + jitter_x;
        cy += fan_y + jitter_y;
