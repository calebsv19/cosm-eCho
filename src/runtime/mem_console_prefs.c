...
int mem_console_build_app_prefs_path(char *out_path, size_t out_cap) {
    ...
    written = snprintf(out_path, out_cap, "%s/mem_console.app.pack", data_dir);
    if (written <= 0 || (size_t)written >= out_cap) {
        out_path[0] = '\0';
        return 0;
    }
...
CoreResult mem_console_app_prefs_load(const char *prefs_path, char *db_path, size_t db_path_cap) {
    CorePackReader reader = {0};
    CorePackChunkInfo chunk = {0};
    CoreResult result;
    MemConsoleAppPrefsV1 prefs = {0};
    FILE *probe = 0;

    if (!prefs_path || !db_path || db_path_cap == 0u) {
    ...
    if (prefs.version != MEM_CONSOLE_APP_PREFS_VERSION || prefs.last_db_path[0] == '\0') {
        return core_result_ok();
    }
...
CoreResult mem_console_app_prefs_save(const char *prefs_path, const char *db_path) {
    CorePackWriter writer = {0};
    CoreResult result;
    MemConsoleAppPrefsV1 prefs = {0};
