...
int mem_console_build_app_prefs_path(char *out_path, size_t out_cap) {
    ...
    return mem_console_build_app_prefs_path_for_output_root(data_dir, out_path, out_cap);
}
...
CoreResult mem_console_app_prefs_load(const char *prefs_path,
    ...
    if ((prefs.version != 1u && prefs.version != MEM_CONSOLE_APP_PREFS_VERSION) || prefs.last_db_path[0] == '\0') {
        return core_result_ok();
    }
