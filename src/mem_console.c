...
static void apply_compact_ui_density(KitUiContext *ui_ctx) {
    if (!ui_ctx) {
        return;
    }
...
static void apply_pending_action(CoreMemDb *db,
    ...
    if (action == MEM_CONSOLE_ACTION_CENTER_GRAPH) {
        if (!state->graph_mode_enabled) {
            (void)snprintf(state->status_line,
                           sizeof(state->status_line),
                           "Enable graph mode before centering.");
            mem_console_redraw_mark(state, MEM_CONSOLE_REDRAW_REASON_CONTENT);
            return;
        }
        ...
        if (result.code != CORE_OK) {
            set_action_error_status(state, "Center graph failed", result);
            return;
        }
    ...
    if (action == MEM_CONSOLE_ACTION_CENTER_SELECTED) {
        if (!state->graph_mode_enabled) {
            (void)snprintf(state->status_line,
                           sizeof(state->status_line),
                           "Enable graph mode before focusing node.");
            mem_console_redraw_mark(state, MEM_CONSOLE_REDRAW_REASON_CONTENT);
            return;
        }
        ...
        if (result.code != CORE_OK) {
            set_action_error_status(state, "Center selected failed", result);
            return;
        }
